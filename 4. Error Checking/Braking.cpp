...
int main(const int argc, const char* const argv[])
{
    ...
    else
    {
        ...
        {
            cout << "There is no possibility of collision because the car is moving away from the "
                    "obstacle."
                 << endl;
        }
        ...
        else
        {
            ...
            if (minDec <= maxDec)
            {
                float maxT = abs(v) / abs(minDec); // maximum stopping time with minimum
                                                   // deceleration
            ...
            {
                float crashV = sqrt(pow(v, 2)
                    - 2 * abs(d)
                        * abs(maxDec)); // positive velocity at crash with maximum deceleration
                float crashT = (abs(v) - abs(crashV))
                    / abs(maxDec); // time before imminent crash with positive velocity
                cout << "Crash will occur in " << crashT << " seconds at velocity " << crashV
                     << "m/s; deploying airbags." << endl;
            }
        }
    }
}
